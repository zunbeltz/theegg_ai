...
\node [proc, densely dotted, it] (p0) {Pokemon battle};
% Use join to connect a node to the previous one 
\node [proc, join]      {\lstinputlisting[language=Python]{initialize_code.py}};
\node [proc, join] (p1) {$\text{turn} \gets 0$};
% \node [proc, join]      {Open queue};
% \node [proc, join]      {Dispatch message};
\node [test, join] (t1) {$\text{turn} = 0$?};
% No join for exits from test nodes - connections have more complex
% requirements
% We continue until all the blocks are positioned
\node [proc] (p2) {p2.life =- p1.attack};
